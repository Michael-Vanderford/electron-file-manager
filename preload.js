...
// /home/michael/source/repos/electron-file_manager/assets/icons/korla/symbolic/image-x-generic-symbolic.svg
...
// todo: // use this for main files array
// let dirents = []
...
ipcRenderer.on('copy_files', (e, files) => {
    ...
    if (files.copy_files_arr.length > 0) {
        ...
        files.copy_files_arr.forEach((file, idx) => {
            ...
            console.log('copy files array ' + source_folder)
            ...
            if (fs.existsSync(file.source) == true) {
                ...
                console.log('source folder ' + destination)
                ...
                if (source_folder == destination_folder) {
                    ...
                    if (stats.isDirectory()) {
                        ...
                        interval_id = setInterval(() => {
                            ...
                            ++c
                            console.log('running get progress ' + c)
                            get_progress(folder_size, root, c)
                ...
                // NO DUPLICATES FOUND. COPY FILE / DIR
                    ...
                    // FILES
                        ...
                        if (fs.existsSync(destination)) {
                            ...
                            let msg = destination + ' file exists.  confirm overwrite'
                            ...
                            ipcRenderer.on('overwrite_confirmed', (e, res) => {
                                ...
                                copy_files(file.source, path.join(destination))
...
ipcRenderer.on('gio_devices', (e, res) => {
    ...
    for (let i = 0; i < volumes_arr.length; i++) {
        ...
        data.forEach((subitem, subidx) => {
            ...
            // console.log(volumeidx + ' ' + volumeidx2)
            ...
            if (subidx >= volumeidx && subidx <= volumeidx2) {
                ...
                if (subitem.indexOf('default_location=') > -1) {
                    ...
                    // console.log(fs.realpathSync(uuid))
                    ...
                    href.href = '#'
                    href.classList.add('header_link', 'block')
                ...
                if (subitem.indexOf('activation_root=') > -1) {
                    ...
                    let uuid = subitem.replace('activation_root=', '').trim()
                    gio.uuid = uuid
                    ...
                    href.addEventListener('click', (e) => {
                        ...
                        // ipcRenderer.send('get_gio_files', uuid)
...
function get_progress(source_size, destination_file, c) {
    ...
    if (stats.isDirectory()) {
        ...
        ipcRenderer.on('folder_size', (e, data) => {
            ...
            console.log('current size ' + current_size, ' folder size ', source_size)

            transfer_speed = get_transfer_speed(current_size, c, source_size)
            // progress.max = Math.floor(transfer_speed.transfer_time)
            ...
            if (progress) {
                ...
                progress.value = transfer_speed.transfer_speed
    ...
    // FILE
        ...
        current_size = localStorage.getItem(destination_file)
        console.log('current size ' + current_size, ' folder size ', folder_size)

        transfer_speed = get_transfer_speed(current_size, c, source_size)

        if (progress) {

            console.log('transfer time ' +  Math.floor(transfer_speed.transfer_time))
            // progress.max = Math.floor(transfer_speed.transfer_time)
...
// GET TRANSFER SPEED
function get_transfer_speed(current_size, elapsed_time, folder_size) {

    let transfer_speed = get_raw_file_size(current_size) / elapsed_time
    let transfer_time = get_raw_file_size(folder_size) / transfer_speed
    let transfer_data_amount = transfer_speed * transfer_time

    console.log('current size ' + get_raw_file_size(current_size))
    console.log('elapsed speed ' + elapsed_time)

    console.log('transfer speed ' + transfer_speed)
    console.log('transfer time ' + transfer_time)
    console.log('transfer amount ' + transfer_data_amount)
    ...
    try {
        ...
        if (grid.id == 'search_results' || grid.id == 'folder_grid' || grid.id == 'workspace_grid') {
            ...
            if (cardindex > 1) {
                ...
                img.src = '../assets/icons/korla/actions/scalable/image-x-generic-symbolic.svg'
        ...
        header.addEventListener('mouseover', function (e) {
            ...
            if (is_folder) {
                ...
                card_id = id
                img.src = '../assets/icons/korla/places/scalable/folder-black-drag-accept.svg'
            }
        ...
        card.ondragstart = function (e) {
            ...
            var f = new File([""], linktext, { type: get_file_type(href), lastModified: '' })
...
// GET FOLDER SIZE
async function get_folder_size(dir) {

    notification('running get folder size')

    // GET TEMPLATE FILES FROM THE TEMPLATES DIRECTORY
    // todo: this
    // get_templates()
...
async function get_files(dir) {
    ...
    fs.readdir(dir, (err, dirents) => {
        ...
        if (dirents.length > 0) {
            ...
            // DONT SORT IF FOLDER IS GVFS. ITS TOO SLOW. AHHH
            // todo: handle gvfs better
            ...
            // // PAGE FILES
            // if(filter.length > pagesize){
            //     let number_pages = parseInt(parseInt(filter.length) / parseInt(pagesize))
            //     // console.log(parseInt(number_pages))
            //     for(let i = 1; i < number_pages + 1; i++){
            //         add_pager_item({dir, name: i})
            //     }
            //     filter = paginate(filter, pagesize, options.page)
            //     // console.log('running')
            // }
            ...
            // FILTER DIRECTORIES ///////////////////////////////////////////////////
            let dirs = filter.filter(function (e) {
                let s1 = stat.statSync(path.join(dir, e))
                if (s1) {
                    return s1.isDirectory() === true
                }
            })

            // THIS IS FOR NAVIGATION. DO NOT DELETE!
            ...
            // RETURN DIRECTORIES
            let directories = dirs.filter((hidden) => !hidden.match(regex))

            if (directories.length > 0) {

                console.log('setting folder_count to ' + directories.length)
                folder_count = directories.length

                // LOOP OVER DIRECTORIES ARRAY
                directories.forEach((file, idx) => {
            ...
            let hidden_folders_card = document.getElementById('hidden_folders_card')
            // hidden_directories = directories.filter((hidden) => hidden.match(regex))
            let hidden_directories = dirs.filter((hidden) => hidden.match(regex))

            if (hidden_directories.length > 0) {

                hidden_folder_count = hidden_directories.length

                // LOOP OVER DIRECTORIES ARRAY
                hidden_directories.forEach((file, idx) => {
            ...
            if (filter.length > pagesize) {
                ...
                filter = paginate(filter, pagesize, page)

            }


            // FILTER ON FILES
            let filter_files = filter.filter(function (e) {

                let s1 = stat.statSync(dir + '/' + e)
                return s1.isDirectory() == false

            })


            // RETURN FILES
            let files = filter_files.filter((x) => !x.match(regex))
            ...
            // RETURN HIDDEN FILES
            let hidden_files = filter_files.filter((x) => x.match(regex))
            if (hidden_files.length > 0) {

                hidden_file_count = hidden_files.length

                // LOOP OVER FILES ARRAY
                hidden_files.forEach((file, idx) => {
            ...
            main_view.addEventListener('keypress', function (e) {
                ...
                if (regex.test(e.key) === false && !e.shiftKey && e.key !== 'Delete') {
    ...
    Mousetrap.bind('right', (e) => {
        ...
        let count = main_view.querySelectorAll('.nav_item').length
    ...
    Mousetrap.bind('down', (e) => {
        ...
        // let card = main_view.querySelector('.nav_item')
        ...
        console.log('this should be 19 len ' + folder_cards.length + ' ' + nc)

        if (nc2 == 0) {

            // console.log('running')
            document.querySelector("[data-id='" + (nc) + "']").classList.add('highlight_select')
            document.querySelector("[data-id='" + (nc) + "']").querySelector('a').focus()
            ...
            // THIS KEEPS FROM GOING OVER NUMBER OF CARDS
            // if (nc2 - 5  < (folder_cards.length - 5 + file_cards.length - 5)) {
            // if (nc2  < ((folder_cards.length - 5) + (file_cards.length - 5) - hidden_folder_count - 5)) {

            // THIS KEEPS FROM GOING OVER NUMBER OF CARDS
            console.log(items.length)
            if (nc2 + 5 < items.length) {

                console.log('folder count ' + folder_count)
                console.log('hidden folder count ' + hidden_folder_count)

                console.log('file count ' + file_count)
                console.log('hidden file count ' + hidden_file_count)
                ...
                if (nc2 > 1) {
                    ...
                    console.log('adjustment is ' + adj)

                    // IF NAVIGATION COUNTER > ADJUSTMENT
                    if (nc2 > adj && is_folder_card == true) {

                        console.log('setting is folder card to false')
                        is_folder_card = false
                ...
                // RESET NC TO LAST ITEM AND NC2 TO 1
                nc = nc2
                nc2 = 1
    ...
    Mousetrap.bind('up', (e) => {
        ...
        if (nc2 > 5) {
            ...
            if (nc - 4 < folder_cards.length && is_folder_card == false) {

                is_folder_card = true

                let last_row_count = Math.ceil(folder_cards.length / 5) * 5
                let adj = folder_cards.length - last_row_count

                console.log('next one is folder')
                console.log('last ' + last_row_count)
                console.log('adj ' + adj)
                console.log('nc ' + nc)

                nc2 = nc2 - adj
                console.log('this should be 21 ' + nc2)
...
window.addEventListener('contextmenu', function (e) {
    ...
    if (target) {
        ...
        let filetype = get_file_type(source)
...
function add_grid() {
    ...
    grid.classList.add('ui', 'grid', 'stackable',)
    return grid
}
...
function get_icon_path(file) {
    ...
    if (stats.isDirectory()) {
        ...
        if ((path.basename(breadcrumbs)) == 'gvfs') {
            icon = '../assets/icons/korla/apps/scalable/gnome-device-manager.svg'
        } else {
            icon = '../assets/icons/korla/places/scalable/gnome-folder.svg'
        }
        ...
        if (file_ext.toLocaleLowerCase() == '.jpg' || file_ext.toLocaleLowerCase() == '.png' || file_ext.toLocaleLowerCase() == '.jpeg' || file_ext.toLocaleLowerCase() == '.gif' || file_ext.toLocaleLowerCase() == '.svg' || file_ext.toLocaleLowerCase() == '.ico' || file_ext.toLocaleLowerCase() == '.webp') {
            ...
            icon = '../assets/icons/korla/apps/scalable/ms-excel.svg' //../assets/icons/korla/apps/scalable/libreoffice-calc.svg'
        } else if (file_ext == '.docx' || file_ext == '.ott' || file_ext == '.odt') {
            icon = '../assets/icons/korla/apps/scalable/libreoffice-writer.svg'
        } else if (file_ext == '.wav' || file_ext == '.mp3' || file_ext == '.mp4' || file_ext == '.ogg') {
            icon = '../assets/icons/korla/mimetypes/scalable/audio-wav.svg'
        } else if (file_ext == '.iso') {
            icon = '../assets/icons/korla/apps/scalable/isomaster.svg'
        } else if (file_ext == '.pdf') {
            icon = '../assets/icons/korla/apps/scalable/gnome-pdf.svg'
        } else if (file_ext == '.zip' || file_ext == '.xz' || file_ext == '.tar' || file_ext == '.gz' || file_ext == '.bz2') {
            icon = '../assets/icons/korla/apps/scalable/7zip.svg'
        } else if (file_ext == '.deb') {
            icon = '../assets/icons/korla/apps/scalable/gkdebconf.svg'
        } else if (file_ext == '.txt') {
            icon = '../assets/icons/korla/apps/scalable/text.svg'
        } else if (file_ext == '.sh') {
            icon = '../assets/icons/korla/apps/scalable/terminal.svg'
        } else if (file_ext == '.js') {
            icon = '../assets/icons/korla/apps/scalable/applications-java.svg'
        } else if (file_ext == '.sql') {
            icon = '../assets/icons/korla/mimetypes/scalable/application-x-sqlite.svg'
        } else {
            icon = '../assets/icons/korla/mimetypes/scalable/application-document.svg'
        }
    ...
    cards.forEach((card, idx) => {
        ...
        if (stats.isDirectory()) {
            ...
            // if (href.indexOf('/gvfs') === -1) {
                ...
                img.src = '../assets/icons/korla-pgrey/places/scalable@2/network-server.svg'
                img.height = '24px'
                img.width = '24px'
...
function extract(source) {
    ...
    switch (ext) {
        case '.zip':
            ...
            cmd = 'cd "' + breadcrumbs.value + '"; /usr/bin/unzip "' + source + '"'
            break;
        case '.tar':
